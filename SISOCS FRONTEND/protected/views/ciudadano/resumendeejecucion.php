...
//$IdProyecto = 147;
 ...
 from cs_proyecto p, cs_programa pro where pro.idPrograma = $IdPrograma and pro.idPrograma = p.idPrograma and p.idProyecto=$IdProyecto;";
		...
		$sql3="select distinct ca.idProyecto,
c.idContratacion,
a.idAdjudicacion,
ca.nomprocesoproyecto as titulocontrato,
(select nombre_empresa from cs_empresa e where e.idempresa = c.idoferente) as Firma,
case when
(select co.precioactual 
 from cs_contratos co where  c.idContratacion = co.idContratacion order by co.fechatercontra desc limit 1) IS NULL THEN c.precio
else 
(select co.precioactual 
 from cs_contratos co where  c.idContratacion = co.idContratacion order by co.fechatercontra desc limit 1) end
 as Monto_Lps_Ultimo,
...
ca.tipocontrato,
case when
(select co.fechatercontra 
 from cs_contratos co where  c.idContratacion = co.idContratacion order by co.fechatercontra desc limit 1) IS NULL THEN c.fechafinal
else 
(select co.fechatercontra 
 from cs_contratos co where  c.idContratacion = co.idContratacion order by co.fechatercontra desc limit 1) end
 as FechaFinalActualizada


from cs_calificacion ca,cs_contratacion c,cs_adjudicacion a
where ca.idProyecto = $IdProyecto and ca.tipocontrato  like 'Cons%' and ca.idCalificacion = a.idCalificacion
and a.idAdjudicacion = c.idAdjudicacion
 order by FechaFinalActualizada desc limit 1;";
...
$sql4="select distinct ca.idProyecto,
c.idContratacion,
a.idAdjudicacion,
ca.nomprocesoproyecto as titulocontrato,
(select nombre_empresa from cs_empresa e where e.idempresa = c.idoferente) as Firma,
...
from cs_calificacion ca,cs_contratacion c,cs_adjudicacion a
where ca.idProyecto = $IdProyecto and ca.tipocontrato not like 'Cons%' and ca.idCalificacion = a.idCalificacion
and a.idAdjudicacion = c.idAdjudicacion
 order by FechaFinalActualizada desc limit 1;";
...
<table  class="tg">
     <td width="70%" valign="top" class="tg-031e"  style="text-align:left;">
         <table width="100%">
             <tr>
              <td width="10%"><strong>Ente Responsable:<strong></td>
...
<table  class="tg">
	...
	<?php  
            ...
            /*
			$sql = "Select
                      cs_proyecto_beneficiario.idproyecto As idProyecto,
                      cs_beneficiario.idbeneficiarios As idBeneficiarios,
                      cs_beneficiario.depto As codigoDepto,
                      cs_beneficiario.muni As codigoMuni,
                      cs_departamento.departamento As Departamento,
                      cs_municipio.municipio As Municipio,
                      cs_proyecto_beneficiario.km As KM
                    From
                      cs_proyecto_beneficiario Inner Join cs_beneficiario On 
                        cs_proyecto_beneficiario.idbeneficiario = cs_beneficiario.idbeneficiarios Inner Join cs_municipio On 
                            cs_beneficiario.depto = cs_municipio.depto And 
                            cs_beneficiario.muni = cs_municipio.id Inner Join cs_departamento On 
                                cs_beneficiario.depto = cs_departamento.codigoDepto
                    Where
                      cs_proyecto_beneficiario.idproyecto = $idProyecto;";
            ...
            */
			...
			$result = mysql_query("Select
                                      ...
                                      cs_proyecto_beneficiario.km As KM
                                    From
                                      cs_proyecto_beneficiario Inner Join
                                      cs_beneficiario On cs_proyecto_beneficiario.idbeneficiario =
                                        cs_beneficiario.idbeneficiarios Inner Join
                                      cs_municipio On Cast(cs_beneficiario.depto As Unsigned) =
                                        Cast(cs_municipio.depto As Unsigned) And Cast(cs_beneficiario.muni
                                        As Unsigned) = Cast(cs_municipio.id As Unsigned) Inner Join
                                      cs_departamento On Cast(cs_beneficiario.depto As Unsigned) =
                                        Cast(cs_departamento.codigoDepto As Unsigned)
                                    Where
                                      cs_proyecto_beneficiario.idproyecto = $idProyecto;");
...
<table  class="tg">
     <td width="50%" valign="top" class="tg-031e"  style="text-align:left;">
         <table width="100%">
             ...
             <tr>
               <td width="10%"><strong>Consultor/ Firma Contratada:<strong></td>
             <td> <?php echo $S_Firma_Contrato ?> </td> 
             </tr>
              <tr>
               <td width="10%"><strong>Monto Contratado (LPS):<strong></td>
             <td>  <?php echo  number_format($S_Monto_Lps_Contrato,2) ; ?></td> 
             </tr>
              <!--tr>
...
<td class="tg-031e"  colspan="3"></td>    
 ...
 <!--div class="slideshow" 
    ...
    $result = mysql_query("select * from v_proyectos_carrousel where idProyecto = ".$IdProyecto." Limit 5");
			...
			while($row = mysql_fetch_array($result))
			{			
			echo "<a href=".$row['ubicacion_img']." target=\"_blank\"><img src='" . $row['ubicacion_img'] . "' style='width:200px;height:200px';  ></a>";		
