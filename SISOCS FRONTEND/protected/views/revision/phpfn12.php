...
function CurrentProjectID() {
	...
	return "{95DDD5E1-EED3-4F75-9459-65662A38CD3B}";
}
...
function ew_GetFileUploadUrl($fld, $val, $resize = FALSE, $encrypt = EW_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ew_EmptyStr($val)) {
		$path = ew_UploadPathEx(FALSE, $fld->UploadPath);
		if ($encrypt) {
...
class cAdvancedSecurity {
	...
	function UserLevelNameList() {
		...
		foreach ($this->UserLevelID as $UserLevelID) {
			if ($list <> "") $list .= ", ";
			$list .= ew_QuotedValue($this->GetUserLevelName($UserLevelID), EW_DATATYPE_STRING, EW_USER_LEVEL_TABLE_DBID);
		}
		return $list;
	}
