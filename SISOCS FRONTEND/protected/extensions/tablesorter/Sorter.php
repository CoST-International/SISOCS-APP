...
class Sorter extends CWidget
{
	...
	public function genTable()
	{
		...
		foreach($datas as $data)
		{
			...
			$view_url=Yii::app()->createAbsoluteUrl($class.'/view',array('id'=>$data->primaryKey));
			$edit_url=Yii::app()->createAbsoluteUrl($class.'/update',array('id'=>$data->primaryKey));
			$delete_url='"'.Yii::app()->createAbsoluteUrl($class.'/delete',array('id'=>$data->primaryKey)).'"';
			$del_data='"'.$data->primaryKey.'"';
				...
				if($buttons[0]['action']!='disable')
				{
					if(isset($buttons))
					{
						...
						if(isset($buttons[0]['view'])) {
							...
							echo "<a class='btn btn-small btn-green' href='".$view_url."'><i class='icon-search'></i></a> &nbsp;"; 
						} else {
							echo "<a class='btn btn-small btn-green' href='".$view_url."'><i class='icon-search'></i></a> &nbsp;"; 
						}
						if(isset($buttons[0]['edit'])) {
							if($buttons[0]['edit']!='disable')
							echo "<a class='btn btn-small btn-yellow' href='".$edit_url."'><i class='icon-edit'></i></a>  &nbsp;"; 
						} else {
							echo "<a class='btn btn-small btn-yellow' href='".$edit_url."'><i class='icon-edit'></i></a>  &nbsp;"; 
						}	
						if(isset($buttons[0]['delete'])) {
							if($buttons[0]['delete']!='disable')
							echo "<a class='btn btn-small btn-red' href='javascript:void(0);' onclick='del_data(".$delete_url.");'><i class='icon-trash'></i></a>";
						} else {
							echo "<a class='btn btn-small btn-red' href='javascript:void(0);' onclick='del_data(".$delete_url.");'><i class='icon-trash'></i></a>";
						}	
						echo "</td>";
					...
					{
						echo "<td>   <a class='btn btn-small btn-green' href='".$view_url."'><i class='icon-search'></i></a>
							  &nbsp; <a class='btn btn-small btn-yellow' href='".$edit_url."'><i class='icon-edit'></i></a>
							  &nbsp; <a class='btn btn-small btn-red' href='javascript:void(0);' onclick='del_data(".$delete_url.");'><i class='icon-trash'></i></a>
							  </td>";
					}
				}	
			} else {
