...
//		);
 return array('accessControl',array('CrugeAccessControlFilter'));
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','ViewDetempresa','ViewDetoferentes','ViewDetfirmas','DeleteGet','GeneratePdf','GenerateExcel','FuncionarioRespond','TraerAdquisicion'),
				'users'=>array('*'),
			),
	...
	{
	    $result_=  Yii::app()->db->createCommand()
                                           ->select('*')
                                           ->from('cs_calificacion')
                                           ->where('idCalificacion=:id',array(':id'=>$_GET['id']))										   
                                           ->queryAll();
        if($result_[0]['tipocontrato']=='Mantenimiento'|| $result_[0]['tipocontrato']=='Construcción')
        {
		...
		echo '
				<table class="tablesorter table table-bordered hasFilters tablesorter-bootstrap" witdh="34%">
            <colgroup>
                <col style="width: 8%;">
                <col style="width: 9%;">
                <col style="width: 6%;">
            ...
            <div class="tablesorter-wrapper" style="position:relative;height:100%;width:100%">
                <div class="tablesorter-header-inner">Código Calidicación<i class="tablesorter-icon bootstrap-icon-unsorted"></i></div>
            </div>
            </th>
            <th style="-moz-user-select: none;" unselectable="on" tabindex="0" data-column="2" class="tablesorter-header bootstrap-header">
            <div class="tablesorter-wrapper" style="position:relative;height:100%;width:100%">
                <div class="tablesorter-header-inner">Oferente / Firma <i class="tablesorter-icon bootstrap-icon-unsorted"></i></div>
            </div>
            </th>
			<th style="-moz-user-select: none;" unselectable="on" tabindex="0" data-column="2" class="tablesorter-header bootstrap-header">
            <div class="tablesorter-wrapper" style="position:relative;height:100%;width:100%">
                <div class="tablesorter-header-inner">Accciones<i class="tablesorter-icon bootstrap-icon-unsorted"></i></div>
            </div>
            </th>
        ...
        public function actionViewDetfirmas()
	{
	      $result_=  Yii::app()->db->createCommand()
                                           ...
                                           ->where('idCalificacion=:id',array(':id'=>$_GET['id']))										   
                                           ->queryAll();
        if($result_[0]['tipocontrato']=='Diseño'|| $result_[0]['tipocontrato']=='Diseño y Supervision' || $result_[0]['tipocontrato']=='Supervision')
        {
		...
		echo '
            ...
            <thead>
                <tr class="tablesorter-headerRow">
                    <th style="-moz-user-select: none;" unselectable="on" tabindex="0" data-column="0" class="tablesorter-header bootstrap-header">
            <div class="tablesorter-wrapper" style="position:relative;height:100%;width:100%">
                <div class="tablesorter-header-inner">Código de calificación 	<i class="tablesorter-icon bootstrap-icon-unsorted"></i></div>
            </div>
            </th>
            <th style="-moz-user-select: none;" unselectable="on" tabindex="0" data-column="2" class="tablesorter-header bootstrap-header">
            <div class="tablesorter-wrapper" style="position:relative;height:100%;width:100%">
	...
	public function actionFuncionarioRespond() {
                 
                      $resultXY=  Yii::app()->db->createCommand()
                                           ->select('*')
                                           ->from('cs_funcionarios')
                                           ->where('idfuncionario=:id',array(':id'=>$_GET['id']))										   
                                           ->queryAll();
                    ...
                    echo  '<input type="text" name="puesto"readonly="true" value="'.$resultXY[0]['puesto'].'">';
                    echo   '</br>';
                    echo    '</br>';
                    echo    '<strong>Telefono</strong></br>';
                    echo    '<input type="text" name="telefono" readonly="true" value="'.$resultXY[0]['telefono'].'">';
                    echo    '</br>';
                    echo    '</br>';
                    echo    '<strong>Correo</strong></br>';
                    echo    '<input type="text" name="correo"  readonly="true" value="'.$resultXY[0]['correo'].'">';
                    echo   '</form>';
                    echo   '</div>';
           
        }
		
		  public function actionTraerAdquisicion(){          
            if($_GET['id']=='Construcción' || $_GET['id']=='Mantenimiento'){ 
         ...
         //  echo '<option value="">--Seleccione un valor--</option>';           
            foreach($list as $value=>$name)
             ...
             else if($_GET['id']=='Diseño' || $_GET['id']== 'Supervision' || $_GET['id'] == 'Diseño y Supervision' )
            {
