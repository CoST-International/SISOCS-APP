...
class ContractDocumentsController extends Controller
{
	...
	{
		$model=new ContractDocuments;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['ContractDocuments']))
		{
			$model->attributes=$_POST['ContractDocuments'];
			if($model->save()){
				try {
				...
				$id  = $model->id;
				$dir = Yii::app()->Controller->GetPath('webroot.adjuntos', 1, $id, 'ContractDocuments');
	...
	{
		$dataProvider=new CActiveDataProvider('ContractDocuments');
	...
	{
		$model=new ContractDocuments('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ContractDocuments']))
			$model->attributes=$_GET['ContractDocuments'];
	...
	{
		$model=ContractDocuments::model()->findByPk($id);
		if($model===null)
